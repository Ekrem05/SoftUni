...
describe("E2E tests", function () {
  ...
  describe("Sprint Board Tests", () => {
    it("Load Board (loads all in correct categories)", async () => {
      ...
      const [inProgressTask] = await page.$$eval(
        `#in-progress-section li`,
        (t) => t.map((s) => s.textContent)
      );
      const [codeReviewTask] = await page.$$eval(
      ...
      expect(todoTask).to.equal(
        `${firstTask.title}${firstTask.description}Move to In Progress`
      );
      expect(inProgressTask).to.equal(
        `${secondTask.title}${secondTask.description}Move to Code Review`
      );
      expect(codeReviewTask).to.equal(
        `${thirdTask.title}${thirdTask.description}Move to Done`
      );
      expect(doneTask).to.equal(
        `${fourthTask.title}${fourthTask.description}Close`
      );
    });
    ...
    it("Close Task", async () => {
      ...
      await Promise.all([onResponse(), page.click("#done-section li button")]);
